<template>
    ...
    <div separator style="background-color: white; border: none">
        <q-table
        grid
        hide-header
        :data="getDoenca.content"
        :columns="columns"
        :filter="filter"
        ...
        >
            ...
            <q-card class="transition-generic cursor-pointer" style="background-color: white">
                ...
                <q-list no-border>
                    <q-item v-for="col in props.cols.filter(col => col.name !== 'Nome Doenca')" :key="col.name">
                    <q-item-main>
...
export default {
  ...
  computed: {
    getDoenca () {
      return this.$store.getters.getDoencas
    },
    getMedicamento () {
